...
## cannon-es 물리 엔진 주의사항 (레슨09, 50, 53)
...
- **KINEMATIC 이동 오브젝트(발판 등)를 만들 때는 생성 시 모든 축의 좌표를 명시적으로 설정할 것**. 애니메이션 루프에서 "안 바뀌는 축"이라 업데이트를 안 적으면 그 축은 Body 기본값인 원점(0)에 영원히 남는다 — 매 프레임 z/y만 갱신하고 x를 한 번도 안 정한 탓에 캐릭터와 발판이 완전히 다른 위치에 있어 아예 서로 안 만나던 실수가 실제로 있었음(레슨50). 캐릭터가 "부분적으로만" 뚫리면 대개 SAPBroadphase류 라이브러리 버그, "완전히" 뚫리면 좌표 자체가 안 겹치는 단순 실수일 확률이 높음 — 라이브러리를 의심하기 전에 양쪽 바디의 실제 좌표부터 콘솔로 찍어볼 것
- 캐릭터가 KINEMATIC 발판을 "타고" 움직이게 하려면 마찰에 의존하지 말고(위 friction=0 규칙과 충돌) 발판의 프레임간 이동량(delta)을 캐릭터 위치에 직접 더해주는 수동 패런팅 방식을 쓸 것. 발판 위치는 `.position.set()`뿐 아니라 `.aabbNeedsUpdate = true`도 매 프레임 켜줘야 `raycastClosest`(AABB 캐시 기반)가 정확히 인식함 — 단, 물리적 충돌 자체(`world.step()`의 `doBoundingSphereBroadphase`)는 라이브 좌표를 쓰므로 이 플래그와 무관하게 항상 정확함
- **STATIC/KINEMATIC Body는 position 확정 직후 `updateAABB()`를 명시적으로 호출할 것**. cannon-es는 shape를 등록하는 시점(생성자 옵션으로 넘길 때)에 AABB를 한 번 계산해버리는데, 그 시점은 아직 기본 위치(원점)라서 이후 `.position.set()`으로 실제 위치를 옮겨도 `aabbNeedsUpdate`가 이미 `false`로 고정돼 절대 다시 계산되지 않는다. 물리 충돌(`world.step()`의 브로드페이즈는 라이브 좌표 기반이라 문제 없음)은 멀쩡한데 `raycastClosest`(캐시된 AABB 기반)만 그 바디를 계속 못 찾는, 매우 헷갈리는 증상으로 나타남(레슨50에서 목표 지점뿐 아니라 벽·나무 전체에 걸쳐 실제로 겪음 — 바닥만 우연히 원점 근처라 증상이 안 보였음). 바디를 `world.addBody()`하기 직전, position(및 회전 바디는 quaternion)을 다 정한 뒤 `body.updateAABB()`를 한 번 호출하는 걸 습관화할 것
